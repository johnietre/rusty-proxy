...
impl Proxy {
    ...
    async fn handle<S: IOUnpin>(self, client: S, client_addr: SocketAddr) {
        ...
        loop {
            ...
            // TODO: Update cr/cw with client variable updates
            tokio::select! {
                // Client if left
                lr = self.pipe_conns(&mut cr, sw) => match lr {
                    ...
                    Either::Right(res) => match res {
                        Err(e) => {
                            ...
                            logm(Level::Info, format!("server error: {}", e));
                        }
                        Ok(_) => (), // Unreachable
                    }
                }
                // Client is right
                lr = self.pipe_conns(sr, &mut cw) => match lr {
                    Either::Right(res) => match res {
                        Ok(_) => (),
                        Err(e) => {
